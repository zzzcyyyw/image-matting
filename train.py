...
#     fgs = [item[2] for item in batch]
#     bgs = [item[3] for item in batch]
#     imgs = torch.stack(imgs, dim=0)
#     labels = np.stack(labels, axis=0)
#     fgs = np.stack(fgs, axis=0)
#     bgs = np.stack(bgs, axis=0)
#     return imgs, labels, fgs, bgs
...
def train_net(args):
    ...
    # print(train_dataset[12683])
    train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True, num_workers=8)
    # train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True, num_workers=8, collate_fn=custom_collate_fn)
...
def train(train_loader, model, optimizer, epoch, logger):
    ...
    for i, (img, alpha_label) in enumerate(train_loader):
        ...
        # depth_map = depth_map.type(torch.FloatTensor).to(device)                    # zc
        alpha_label = alpha_label.type(torch.FloatTensor).to(device)  # [N, 320, 320]
        # alpha_label = alpha_label.reshape((-1, 2, im_size * im_size))  # [N, 320*320]    原
        alpha_label = alpha_label.reshape((-1, 1, im_size * im_size))  # [N, 320*320]     zc
        ...
        loss = alpha_prediction_loss(alpha_out, alpha_label)      # 原
...
def valid(valid_loader, model, logger):
    ...
    for img, alpha_label in valid_loader:
        ...
        alpha_label = alpha_label.type(torch.FloatTensor).to(device)  # [N, 320, 320]
        # alpha_label = alpha_label.reshape((-1, 2, im_size * im_size))  # [N, 320*320]     # 原
        alpha_label = alpha_label.reshape((-1, 1, im_size * im_size))       # zc
