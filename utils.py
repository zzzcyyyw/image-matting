...
zc
"""
# def depth_safe_crop(mat, x, y, crop_size=(im_size, im_size)):
#     crop_height, crop_width = crop_size
#     # if len(mat.shape) == 2:
...
def alpha_prediction_loss(y_pred, y_true):
    diff = y_pred[:, 0, :] - y_true[:, 0, :]
    # num_pixels = y_true.size(0) * y_true.size(2)
    num_pixels = torch.tensor(y_true.size(0) * y_true.size(2), dtype=torch.float32, device=y_true.device)
    return torch.sum(torch.sqrt(torch.pow(diff, 2) + epsilon_sqr)) / (num_pixels + epsilon)
